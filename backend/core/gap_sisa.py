...
def petunjuk_sisa(parsed: dict, applied: dict | None) -> list[str]:
    ...
    lines = [
        "",
        "═══ DAMPAK BERKAS SEBELUMNYA (BOM_AKSESORIS yang Anda unggah) ═══",
        f"  DITERAPKAN : {t.get('bom_lines', 0)} baris bahan · {t.get('bom_groups', 0)} kelompok · {t.get('bom_models', 0)} model dari {t.get('bom_models_in_file', 0)} model di berkas.",
    ...
    lines += [
        f"  DILEWATI   : {t.get('bom_skipped', 0)} catatan → " + " · ".join(f"{CATEGORIES.get(k, k)} {v}" for k, v in c.items()),
        "  Sheet lain (HARGA_JUAL_SKU, MATERIAL, REKENING, TOKO, GAJI, STOK_AWAL) TIDAK diterapkan pada mode 'hanya BOM' — hanya dicatat.",
        "",
