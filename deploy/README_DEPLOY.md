...
### 4a. Cara tercepat: restore snapshot data klien yang SUDAH disesuaikan (2026-09-17)
`seed/DA_SEED_GOLIVE.archive.gz` di repo = backup VPS 2026-09-17 13:02 WIB **+ BOM_AKSESORIS dari berkas klien sudah
diterapkan** (62 model · 1.547 baris aksesoris · 634 BOM · HPP 63 tervalidasi/41 belum). Dibuat dari DB `dahost_erp`,
jadi langsung cocok dengan `backup.sh restore`:
```bash
...
```
Alternatif tanpa menimpa DB: setelah `update.sh`, unggah `DATA_YANG_PERLU_DIISI_DA.xlsx` klien lewat Portal Keuangan →
Master Akuntansi → Impor Harga · Rekening · BOM → **Terapkan (hanya BOM)** — hasilnya identik (importir idempoten) karena
VPS sudah memuat baseline yang sama. Berkas sisa berikutnya: tombol **"Data yang Perlu Diisi — SISA dari berkas ini"**.
Arsip `seed/vps_baseline_20260917_130216.archive.gz` = kondisi VPS murni sebelum BOM (untuk membatalkan).
